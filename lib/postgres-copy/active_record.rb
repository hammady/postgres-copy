module ActiveRecord
  class Base
    ...
    def self.pg_copy_to path = nil, options = {}
      ...
      sql = options[:sql] || self.scoped.to_sql
      if path
        raise "You have to choose between exporting to a file or receiving the lines inside a block" if block_given?
        connection.execute "COPY (#{sql}) TO #{sanitize(path)} WITH #{options_string}"
      else
        connection.execute "COPY (#{sql}) TO STDOUT WITH #{options_string}"
